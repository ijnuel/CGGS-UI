...
export class SessionEffect {
  ...
  $sessionList = createEffect(() =>
    this.actions$.pipe(
      ...
      switchMap(({ pageQuery }) => {
        ...
        return this.http
          .post<GenericResponseInterface<PaginatedResponseInterface<SessionListInterface[]>>>(
  ...
  $sessionExists = createEffect(() =>
    this.actions$.pipe(
      ...
      switchMap(({ properties }) =>
        this.http
          .get<GenericResponseInterface<boolean>>(`${environment.baseUrl}/Session/Exists`, { params: properties as any, withCredentials: true }
          )
          .pipe(
            map((payload) =>
